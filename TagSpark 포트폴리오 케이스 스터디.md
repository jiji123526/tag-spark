...
- 전체 본문은 검토했으나 함정(길이·문체 과의존·저작권)이 커서 **요약**으로 결정.
- 요약은 FicSim의 실패 원인(문체 과의존)을 구조적으로 회피 — 문체를 제거하고 줄거리·주제만 남기므로.
- 요약으로 Plot·Theme 축이 열림. Style은 무드 지도엔 불필요.
- 결합: **사람 태그(backbone) + 요약(보강) + 쌍 비교(미세 조정).**
- 요약은 축의 재료가 아니라 **태그 검증·누락 발견·콜드스타트** 도구로 사용.
...
| 태그 정합성 | 요약↔태그 일치로 추천자 태그 legit 검증 |
...
- 요약을 통짜 임베딩해 무드를 뽑는 것은 FicSim 경고대로 실패 위험 — 정밀화는 쌍 비교가 담당.
